...
from MissMusic import YouTube, app
from MissMusic.core.call import Miss
from MissMusic.misc import SUDOERS, db
from MissMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from MissMusic.utils.decorators.language import languageCB
from MissMusic.utils.formatters import seconds_to_min
from MissMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from MissMusic.utils.stream.autoclear import auto_clean
from MissMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Miss.pause_stream(chat_id)
        ...
        await Miss.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Miss.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Miss.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Miss.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            run = await CallbackQuery.message.reply_photo(
                ...
                caption=_["stream_1"].format(
                    user,
                    f"https://t.me/{app.username}?start=info_{videoid}",f"https://t.me/KUMiSCooLLL",
                ),
            ...
            try:
                await Miss.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
